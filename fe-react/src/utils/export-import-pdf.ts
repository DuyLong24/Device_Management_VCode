...
export const exportImportPDF = async (data: DeviceImport, extraData?: { originName?: string, deviceTypeName?: string }) => {
    ...
    doc.text(extraData?.originName || (data.origin === 'IMPORT' ? 'Nhập khẩu' : data.origin === 'DOMESTIC' ? 'Nội địa' : data.origin === 'WARRANTY_RETURN' ? 'Trả bảo hành' : data.origin), col2ValueX, y);
