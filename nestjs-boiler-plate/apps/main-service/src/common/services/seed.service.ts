...
export class SeedService implements OnModuleInit {
    ...
    private async seedUsers() {
        const superAdminRole = await this.fncRoleModel.findOne({ code: 'super_admin' });
        if (!superAdminRole) return;

        const adminEmail = 'superadmin@gmail.com';
        const exists = await this.userModel.findOne({ email: adminEmail });

        let targetUser = exists;
        if (!exists) {
            const hashedPassword = await bcrypt.hash('123456789', 10);
            targetUser = await this.userModel.create({
                email: adminEmail,
                password: hashedPassword,
                name: 'Super Admin',
                username: 'superadmin',
                funcRoleId: (superAdminRole._id as any).toString(),
                status: 'ACTIVE',
            });
            this.logger.log(`Created User: ${adminEmail}`);
        } else {
            // Update role if missing
            if (!exists.funcRoleId) {
                exists.funcRoleId = (superAdminRole._id as any).toString();
                await exists.save();
            }
        }

        if (targetUser && !targetUser.keycloakId) {
            try {
                const keycloakId = await this.userKeycloakIntegrationService.syncUserToKeycloak({
                ...
                if (keycloakId) {
                    targetUser.keycloakId = keycloakId;
                    await targetUser.save();
                    this.logger.log(`✅ Synced user to Keycloak (ID: ${keycloakId})`);

                    // Assign role
                    await this.userKeycloakIntegrationService.assignRoleInKeycloak(targetUser.email, 'super_admin');
                } else {
                    this.logger.warn(`⚠️ Failed to sync user to Keycloak`);
                }
            } catch (error) {
                this.logger.error(`❌ Error syncing to Keycloak:`, error.message);
            }
        }
    }
